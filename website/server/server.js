...
try {
  ...
  optionalModules.IntegrationManager =
    require('./src/integrations/integrationManager').IntegrationManager;
} catch (error) {
...
if (process.env.NODE_ENV === 'production') {
  ...
  // Trust proxy headers in production (for load balancers, CDNs)
...
const CONFIG = {
  ...
  enableDetailedLogging:
    process.env.DETAILED_LOGGING === 'true' || process.env.NODE_ENV === 'development',
};
...
class ServiceManager {
  ...
  async initialize() {
    ...
    const initTasks = [this.initAnalytics(), this.initIntegrations(), this.initEmail()];
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        console.error(
          `❌ ${serviceNames[index]} initialization failed:`,
  ...
  get integrations() {
    return this.services.integrationManager;
  }
  get email() {
    return this.services.emailTransporter;
  }
...
app.use(
  ...
  createRateLimit(
    ...
    'Too many API requests from your IP, please try again later.'
  )
);
...
app.use((req, res, next) => {
  ...
  createRateLimit(
    ...
    'Too many page requests, please slow down.'
  )(req, res, next);
});
...
function setupRoutes() {
  ...
  app.post(
    '/api/newsletter',
    createRateLimit(
      CONFIG.rateLimits.newsletter,
      'Too many newsletter signups. Please try again later.'
    ),
